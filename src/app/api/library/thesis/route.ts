...
const createThesisSchema = z.object({
  ...
  researcher_name: z.string().max(500, "Total researcher names must not exceed 500 characters").refine(
    (val) => {
  ...
  // Journal fields
  journal_name: z.string().max(255, "Journal name must not exceed 255 characters").optional(),
  journal_volume: z.string().max(50, "Journal volume must not exceed 50 characters").optional(),
  journal_issue: z.string().max(50, "Journal issue must not exceed 50 characters").optional(),
  doi: z.string().max(100, "DOI must not exceed 100 characters").optional(),
  co_authors: z.string().optional(),
  // Thesis fields
  adviser_name: z.string().max(255, "Adviser name must not exceed 255 characters").optional(),
  // Capstone fields
  team_members: z.string().optional(),
  project_type: z.string().max(100, "Project type must not exceed 100 characters").optional(),
  capstone_category: z.string().max(50, "Capstone category must not exceed 50 characters").optional(),
  program: z.string().max(100, "Program must not exceed 100 characters").optional(),
  ...
  try {
    ...
    if (data.document_type !== "Journal") {
      ...
      if (!data.researcher_name || data.researcher_name.trim().length === 0) {
        return NextResponse.json(
          { message: "Researcher name is required for Thesis and Capstone documents." },
          { status: 422 }
        );
      }
    }
    ...
    const finalTitle = data.document_type === "Journal" 
    ...
    const finalResearcherName = data.document_type === "Journal"
    ...
    const thesis = await prisma.lib_thesis_documents.create({
      data: {
        ...
        adviser_name: data.adviser_name ? sanitizeInput(data.adviser_name) : null,
        team_members: data.team_members ? sanitizeInput(data.team_members) : null,
        project_type: data.project_type ? sanitizeInput(data.project_type) : null,
        capstone_category: data.capstone_category ? sanitizeInput(data.capstone_category) : null,
