...
const bookCreationFormSchema = z.object({
  books_name: z
    .string()
...
export function BookCreationForm({ onSuccess }: BookCreationFormProps) {
  ...
  const form = useForm<BookCreationFormValues>({
  ...
  const onSubmit = React.useCallback(
    async (values: BookCreationFormValues) => {
      setError(null);
      ...
      try {
        ...
        if (!result.success) {
          setError(
            result.message || "Failed to create book. Please try again."
          );
          return;
        }
