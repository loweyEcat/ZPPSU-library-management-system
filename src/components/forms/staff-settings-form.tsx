...
export function StaffSettingsForm({ initialData }: StaffSettingsFormProps) {
  ...
  const onProfileSubmit = async (values: ProfileUpdateValues) => {
    ...
    try {
      const response = await fetch("/api/library/staff/profile", {
        method: "PATCH",
        headers: {
  ...
  const onPasswordSubmit = async (values: PasswordChangeValues) => {
    ...
    try {
      const response = await fetch("/api/library/staff/password", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
